---
name: absorb
description: Main orchestrator skill that triggers the 3 isolated tracks (User, Developer, Agent) using the Chunked Writer Pattern with Dynamic Chunking and PDF Export.
---
...
7. **Stage 6 (Compilation):** Once all N writers finish, invoke the Compiler Agent to stitch the N chunks into a single smooth master document using the Rolling Editor technique.
8. **Stage 7 (Review):** Pass the master document to the Review Agent for consistency.
9. **Stage 8 (Grounding):** Pass the reviewed document to the Grounding Agent for final verification.
10. **Stage 9 (Exporter):** Invoke the Exporter Agent to convert the final verified Markdown document into a styled PDF.
11. **Final Output:** Save the final .md and .pdf files to `d:\working docsbsorb_docs\<target>\`. 
