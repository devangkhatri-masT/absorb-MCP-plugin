...
2. **This track's Definition of Done:**
   ...
   - Every checklist item Discovery tagged `STUB` for this chunk is mentioned in the chunk's visible prose, at the point a user would encounter that feature — not only in a citation footnote. If you find a `STUB` item that only appears in a footnote/source-list and nowhere in the actual walkthrough text, that is a required revision: a reader following the guide has no way to know that feature doesn't work yet.
